/* --- XUTF8 ------------------------------------------------------------------------------------------ **
**
** This is a single-header library that implements a set of routines that handle UTF-8 encoded byte
** sequences. It provides very basic functionalities with the idea that the caller wil build upon this
** routines to implement app-specific functionalities without the need to import unnecessary code.
**
** To use this library, you need to import this file like any header file from wherever you want to
** use these functions, but then define UTF8_IMPL in one (and only one) of those files (before importing
** this file). By defining UTF8_IMPL this file will also provide the implementing code for the routines,
** creating collisions if it was defined multiple times inside a compilation unit.
**
** You can tune this library by defining some constants along with UTF8_IMPL. If you don't define them,
** default values will be used. These symbols are:
**   
**   UTF_ASSUME_ASCII
**     If set to 1, the library will assume that the provided UTF-8 strings will be mainly composted
**     by ASCII characters. If the assumption that the strings are manily ASCII, and this is set to 1,
**     the routines will be much faster. The default value is 1.
** 
** ---------------------------------------------------------------------------------------------------- */
...
#endif // #ifndef UTF8_IMPL_ALREADY
#endif // #ifdef  UTF8_IMPL